...
def makeRestCall(district_id, date_p):
    ...
    headers = {}
    conn.request("GET",
                 "/api/v2/appointment/sessions/public/findByDistrict?district_id=" + district_id + "&date=" + date_p,
                 payload,
                 headers)
...
def findVaccineByDistrict(district_id, date1=None, vaccine_type='COVAXIN', pincode=None, name_regex=None, buffer_days=1,
                          age_limit=18, min_capacity=1, dose_type="first"):
    ...
    for buf_days in range(0, buffer_days):
        ...
        if dose_type == "first":
            filter_data = list(
                filter(lambda hospital: hospital['available_capacity_dose1'] >= min_capacity, filter_data))
        elif dose_type == "second":
            filter_data = list(
                filter(lambda hospital: hospital['available_capacity_dose2'] >= min_capacity, filter_data))
        else:
            filter_data = list(
                filter(lambda hospital: hospital['available_capacity_dose'] >= min_capacity, filter_data))
        ...
        if len(filter_data) > 0:
            ...
            for hosp in filter_data:
                print("Hospital name:", hosp['name'], "available_capacity:", hosp['available_capacity'], "pincode:",
                      hosp['pincode'], "area:", hosp['block_name'])
            print("----" * 10)
        else:
...
#
def send_email(email_response, email_address=None):
    if email_address is None:
        email_address = ['varunvt119@gmail.com', 'abhishek2gargcochin@gmail.com', 'cancernav26@gmail.com']
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            findVaccineByDistrict('294', vaccine_type='COVAXIN', buffer_days=5)
            time.sleep(30)
            print("sleeping")
